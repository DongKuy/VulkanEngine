...
void renderer::GraphicsPipeline::createRenderPass(Devices &device, SwapChain &swapChain) {
    ...
    if (vkCreateRenderPass(device.get(), &renderPassInfo, nullptr, &_renderPass) != VK_SUCCESS)
        throw std::runtime_error("랜더 패스를 생성하지 못했습니다.");
}
...
void renderer::GraphicsPipeline::createDescriptorSetLayout(VkDevice &device) {
    ...
    if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &_descriptorSetLayout) != VK_SUCCESS)
        throw std::runtime_error("디스크릅트 세트 레이아웃을 생성하지 못했습니다.");
...
void renderer::GraphicsPipeline::createGraphicsPipeline(VkDevice &device, SwapChain &swapChain, std::string frag, std::string vert) {
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &_graphicsPipeline) != VK_SUCCESS)
        throw std::runtime_error("그래픽 파이프라인 생성을 하지 못했습니다.");
...
VkShaderModule renderer::GraphicsPipeline::createShaderModule(VkDevice &device, const std::vector<char> &code) {
    ...
    if (vkCreateShaderModule(device, &createInfo, nullptr, &shaderModule) != VK_SUCCESS)
        throw std::runtime_error("쉐이더 모듈 생성에 실패했습니다.");
