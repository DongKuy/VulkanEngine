...
void renderer::SwapChain::createSwapChain(GLFWwindow* window, Surface &surface, Devices &devices) {
    ...
    if (vkCreateSwapchainKHR(devices.get(), &createInfo, nullptr, &_swapChain) != VK_SUCCESS)
        throw std::runtime_error("스왑체인을 생성하지 못했습니다.");
...
VkExtent2D renderer::SwapChain::chooseSwapExtent(GLFWwindow *window, const VkSurfaceCapabilitiesKHR &capabilities) {
    if (capabilities.currentExtent.width != UINT32_MAX) {
        ...
        actualExtent.width = std::fmax(capabilities.minImageExtent.width,
                                      std::fmin(capabilities.maxImageExtent.width, actualExtent.width));
        actualExtent.height = std::fmax(capabilities.minImageExtent.height,
                                       std::fmin(capabilities.maxImageExtent.height, actualExtent.height));
